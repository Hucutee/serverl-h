...
module.exports = function (app) {
  app.get(
    "/diachi/getma/:id", async (req, res) =>{
        return await sql.query(
            'select * from dia_chi where ma_nd = "'+req.params.id+'"' ,
            (err, data) => {
                if (err) {
                    console.log(err);
  ...
  app.get(
    "/nguoidung/checkemail/:email", async (req, res) =>{
        return await sql.query(
            'select * from nguoi_dung where email = "'+req.params.email+'"' ,
            (err, data) => {
                if (err) {
                    console.log(err);
                    return res.status(500).send(err);
  ...
  app.get(
    "/sanpham/get/:ten&&:trang", async (req, res) =>{
        return await sql.query(
            'select * from san_pham where ten_sp= '+ "'"+ req.params.ten +"'" + ' limit '  + (-10 +req.params.trang*10) + ",10",
            (err, data) => {
                if (err) {
                    console.log(err);
                    return res.status(500).send(err);
  ...
  app.get(
    "/diachi/add/:mand&&:dc", async (req, res) =>{
        return await sql.query(
            'INSERT INTO `dia_chi` (`ma_nd`, `ten_dc`) VALUES (' 
            + "'" + req.params.mand + "'," + "'" + req.params.dc + "')",
            (err, data) => {
                if (err) {
                    console.log(err);
                    return res.status(500).send(err); 
